...
namespace CHIS
{
    public partial class pharmacist : Form
    {
        ...
        {
            SqlCommand command = new SqlCommand("SELECT * FROM pharmacist WHERE UserCode=@p1 and SecurityCode=@p2 and Approval=1", bgl.baglanti());
            ...
            if (dr.Read())
            {
                ...
                pharmacistManage newForm2 = new pharmacistManage();
                newForm2.Show();
                this.Hide();
            }
            else
        ...
        {
            SqlCommand command = new SqlCommand("insert into pharmacist (UserCode,SecurityCode) VALUES (@p1,@p2)", bgl.baglanti());
            command.Parameters.AddWithValue("@p1", userCode.Text);
            command.Parameters.AddWithValue("@p2", securityCode.Text);
        ...
        {
            SqlCommand command = new SqlCommand("UPDATE pharmacist SET UserCode=@p1, SecurityCode=@p2 where Approval=1", bgl.baglanti());
        ...
        {
            SqlCommand command = new SqlCommand("DELETE FROM pharmacist WHERE UserCode = @P1 AND SecurityCode = @P2 AND Approval = 1", bgl.baglanti());
            command.Parameters.AddWithValue("@P1", userCode.Text);
            command.Parameters.AddWithValue("@P2", securityCode.Text);
