...
namespace CHIS
{
    public partial class systemadmin : Form
    {
        ...
        {
            SqlCommand command = new SqlCommand("SELECT * FROM adminpanel WHERE UserCode=@p1 and SecurityCode=@p2 and Approval=1", bgl.baglanti());
            ...
            if (dr.Read())
            {
                ...
                administratorsUsersDatabase newForm2 = new administratorsUsersDatabase();
                newForm2.Show();
                this.Hide();
            }
            else
        ...
        {
            SqlCommand command = new SqlCommand("insert into adminpanel (UserCode,SecurityCode) VALUES (@p1,@p2)", bgl.baglanti());
            command.Parameters.AddWithValue("@p1", userCode.Text);
            command.Parameters.AddWithValue("@p2", securityCode.Text);
        ...
        {
            SqlCommand command = new SqlCommand("UPDATE adminpanel SET UserCode=@p1, SecurityCode=@p2 where Approval=1", bgl.baglanti());
        ...
        {
            SqlCommand command = new SqlCommand("DELETE FROM adminpanel WHERE UserCode = @P1 AND SecurityCode = @P2 AND Approval = 1", bgl.baglanti());
            command.Parameters.AddWithValue("@P1", userCode.Text);
            command.Parameters.AddWithValue("@P2", securityCode.Text);
