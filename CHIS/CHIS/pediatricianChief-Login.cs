...
namespace CHIS
{
    public partial class pediatricianChief_Login : Form
    {
        public pediatricianChief_Login()
        {
            InitializeComponent();
        }
        ...
        {
            SqlCommand command = new SqlCommand("SELECT * FROM pediatricianChief WHERE UserCode=@p1 and SecurityCode=@p2 and Approval=1", bgl.baglanti());
            ...
            if (dr.Read())
            {
                ...
                pediatricianChief_ALL newForm2 = new pediatricianChief_ALL();
                newForm2.Show();
                this.Hide();
            }
            else
        ...
        {
            SqlCommand command = new SqlCommand("insert into pediatricianChief (UserCode,SecurityCode) VALUES (@p1,@p2)", bgl.baglanti());
            command.Parameters.AddWithValue("@p1", userCode.Text);
            command.Parameters.AddWithValue("@p2", securityCode.Text);
        ...
        {
            SqlCommand command = new SqlCommand("UPDATE pediatricianChief SET UserCode=@p1, SecurityCode=@p2 where Approval=1", bgl.baglanti());
        ...
        {
            SqlCommand command = new SqlCommand("DELETE FROM pediatricianChief WHERE UserCode = @P1 AND SecurityCode = @P2 AND Approval = 1", bgl.baglanti());
            command.Parameters.AddWithValue("@P1", userCode.Text);
            command.Parameters.AddWithValue("@P2", securityCode.Text);
